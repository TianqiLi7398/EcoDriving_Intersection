...
for init_vel in vel_collection:
    ...
    cost_dum_plot = []
    for t0_ in t0_set:
        cost_sto_plot.append(result[result["t0"] == t0_]["sto"].values[0] )
        cost_det_plot.append(result[result["t0"] == t0_]["det"].values[0] )
        cost_dum_plot.append(result[result["t0"] == t0_]["dum"].values[0] )
    # print(cost_sto_plot)
    plt.figure(figsize=(10, 8))
